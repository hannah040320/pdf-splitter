...
if uploaded_file:
    ...
    for supplier, pages in supplier_pages.items():
        with st.expander(f"📁 {supplier} ({len(pages)} page{'s' if len(pages) > 1 else ''})"):
            ...
            if st.button(f"🔍 Split {supplier} by PO", key=supplier):
                ...
                for page_num in range(len(supplier_doc)):
                    ...
                    po_text_preview.setdefault(po, []).append(text[:500])  # capture first part of text for preview
                ...
                for po, po_pages in po_map.items():
                    ...
                    with st.expander(f"📄 {po} (Pages {', '.join(str(p+1) for p in po_pages)})"):
